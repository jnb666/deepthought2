...
func NewNetwork(device num.Device, model string) (*Network, error) {
	n := &Network{}
	...
	n.Network = nnet.New(conf)
	n.queue = num.NewQueue(device, conf.Threads)
	n.InitWeights(n.queue, n.Data["train"].Shape)
	n.Tester = NewTester(n.queue, n.Network, n.Data)
	return n, nil
}
...
func (n *Network) Train(conf nnet.Config, restart bool) {
	...
	if restart {
		n.Network = nnet.New(conf)
		n.Tester.Init(n.queue.Device(), n.Data, conf)
		n.InitWeights(n.queue, n.Data["train"].Shape)
		n.trainData = nnet.NewDataset(n.queue.Device(), n.Data["train"], conf.TrainBatch, conf.MaxSamples)
		n.Epoch = 1
	} else if n.Epoch > 0 {
	...
	n.running = true
	go func() {
		acc := num.NewArray(n.queue.Device(), num.Float32)
		epoch := n.Epoch
		done := false
		start := time.Now()
		for !done {
...
func NewTester(q num.Queue, net *nnet.Network, data map[string]nnet.Data) *Tester {
	t := &Tester{Stats: []nnet.Stats{}, data: []*nnet.Dataset{}, pred: [][]int32{}}
	t.Init(q.Device(), data, net.Config)
	for i, dset := range t.data {
		net.Error(q, dset, t.pred[i])
	}
	return t
}

// Initialise tester at start of run
func (t *Tester) Init(device num.Device, data map[string]nnet.Data, conf nnet.Config) {
	t.Epoch = 0
	t.Stats = t.Stats[:0]
	t.data = t.data[:0]
	t.pred = t.pred[:0]
	for _, key := range nnet.DataTypes {
		if d, ok := data[key]; ok {
			dset := nnet.NewDataset(device, d, conf.TestBatch, conf.MaxSamples)
...
func (t *Tester) Test(q num.Queue, net *nnet.Network, epoch int, loss float64, start time.Time) bool {
	...
	for i, dset := range t.data {
		...
		s.Values = append(s.Values, net.Error(q, dset, pred[i]))
	}
